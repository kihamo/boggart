...
var playerStatusTypeStatusPropEnum []interface{}

func init() {
	var res []string
	if err := json.Unmarshal([]byte(`["stop","play","load","pause"]`), &res); err != nil {
		panic(err)
	}
	for _, v := range res {
		playerStatusTypeStatusPropEnum = append(playerStatusTypeStatusPropEnum, v)
	}
}

const (

	// PlayerStatusStatusStop captures enum value "stop"
	PlayerStatusStatusStop string = "stop"

	// PlayerStatusStatusPlay captures enum value "play"
	PlayerStatusStatusPlay string = "play"

	// PlayerStatusStatusLoad captures enum value "load"
	PlayerStatusStatusLoad string = "load"

	// PlayerStatusStatusPause captures enum value "pause"
	PlayerStatusStatusPause string = "pause"
)

// prop value enum
func (m *PlayerStatus) validateStatusEnum(path, location string, value string) error {
	if err := validate.EnumCase(path, location, value, playerStatusTypeStatusPropEnum, true); err != nil {
		return err
	}
	return nil
}

func (m *PlayerStatus) validateStatus(formats strfmt.Registry) error {
