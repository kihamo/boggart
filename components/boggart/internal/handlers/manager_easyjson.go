		...
		case "mqtt_publishes":
			if in.IsNull() {
				...
				for !in.IsDelim(']') {
					var v1 string
					v1 = string(in.String())
					out.MQTTPublishes = append(out.MQTTPublishes, v1)
					in.WantComma()
				}
				in.Delim(']')
			}
		case "mqtt_subscribers":
			if in.IsNull() {
				...
				for !in.IsDelim(']') {
					var v2 string
					v2 = string(in.String())
					out.MQTTSubscribers = append(out.MQTTSubscribers, v2)
					in.WantComma()
				}
				in.Delim(']')
			}
...
func easyjsonEd74d837EncodeGithubComKihamoBoggartComponentsBoggartInternalHandlers(out *jwriter.Writer, in managerHandlerDevice) {
	...
	}
	{
		...
		if in.MQTTPublishes == nil && (out.Flags&jwriter.NilSliceAsEmpty) == 0 {
			...
			out.RawByte('[')
			for v4, v5 := range in.MQTTPublishes {
				if v4 > 0 {
					out.RawByte(',')
				}
	...
	}
	{
		...
		if in.MQTTSubscribers == nil && (out.Flags&jwriter.NilSliceAsEmpty) == 0 {
			...
			out.RawByte('[')
			for v6, v7 := range in.MQTTSubscribers {
				if v6 > 0 {
					out.RawByte(',')
				}
